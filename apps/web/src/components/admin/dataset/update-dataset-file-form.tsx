...
function VariableList({
  ...
  emptyLabel: string;
  getChangeLabel?: (change: NonNullable<DatasetFileUpdateVariableSummary["changes"]>[number]) => string;
        ...
        {variables.map((variable) => (
          <li
            ...
            <span className="text-muted-foreground text-right">
              {variable.label}
              {getChangeLabel && variable.changes?.length
                ? ` (${variable.changes.map(getChangeLabel).join(", ")})`
                : null}
            </span>
          </li>
        ))}
...
export function UpdateDatasetFileForm({ dataset }: UpdateDatasetFileFormProps) {
  ...
  const buildFormData = () => {
    ...
    if (preview) {
      formData.append("expectedFileHash", preview.file.hash);
      formData.append("expectedStateHash", preview.stateHash);
    }
    return formData;
  };
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      if (!result.success || !result.preview) {
        console.error("Preview dataset update error:", result.error);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!result.success) {
        console.error("Confirm dataset update error:", result.error);
      ...
      {preview ? (
        <Card className="shadow-xs">
          ...
          <CardContent className="space-y-6">
            ...
            <section className="space-y-3">
              <h3 className="font-semibold">
                {t("preview.matchedVariables", { count: preview.matchedVariables.length })}
              </h3>
              <p className="text-muted-foreground text-sm">{t("preview.preservedFields")}</p>
              <VariableList
                emptyLabel={t("preview.noMatchedVariables")}
                variables={preview.matchedVariables}
                getChangeLabel={(change) => {
                  if (change === "label") return t("preview.changes.label");
                  if (change === "measure") return t("preview.changes.measure");
                  if (change === "type") return t("preview.changes.type");
                  return t("preview.changes.valueLabels");
                }}
              />
            </section>
