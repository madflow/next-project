...
export function useChartSelectionSectionController({
  ...
  const selectedSplitVariables = splitVariablesBySelection[selectionKey] ?? {};
  ...
  const getStatsForVariable = useCallback(
    (variableName: string) => splitStatsData[variableName] || baseStatsData[variableName] || null,
    [baseStatsData, splitStatsData]
  );
