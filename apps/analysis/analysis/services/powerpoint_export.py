...
def _set_chart_title(slide: Any, title: str, meta_line: str) -> None:
    ...
    _apply_export_font(title_paragraph.font, size=TITLE_FONT_SIZE_PT, bold=True)
    ...
    _apply_export_font(meta_paragraph.font, size=META_FONT_SIZE_PT)
...
def _style_data_label(data_label: Any, text_color: RGBColor) -> None:
    ...
    _apply_export_font(font, size=DATA_LABEL_FONT_SIZE_PT, color=text_color)
...
def _style_value_axis(
    ...
    _apply_export_font(tick_labels.font, size=AXIS_FONT_SIZE_PT)


def _style_category_axis(chart: Any, reverse_order: bool = False) -> None:
    category_axis = chart.category_axis
    category_axis.reverse_order = reverse_order
    _apply_export_font(category_axis.tick_labels.font, size=AXIS_FONT_SIZE_PT)
...
def _add_distribution_chart(
    ...
    *,
    ...
    chart_data.add_series(EMPTY_SERIES_NAME, [point["value"] for point in points])
...
def _add_stacked_bar_chart(slide: Any, rows: Sequence[ExportStackedRow]) -> None:
    ...
    first_row = rows[0]
    chart_data = CategoryChartData(number_format=PERCENT_NUMBER_FORMAT)
    ...
    for segment_index, segment in enumerate(first_row["segments"]):
        ...
        chart_data.add_series(
            segment["label"], values, number_format=PERCENT_NUMBER_FORMAT
        )
...
def _add_pie_chart(slide: Any, points: Sequence[ExportPoint]) -> None:
    ...
    _apply_export_font(
        data_labels.font,
        size=DATA_LABEL_FONT_SIZE_PT,
        color=_best_text_color(SLIDE_BACKGROUND_HEX),
    )
...
def _add_mean_bar_chart(
    ...
    chart_data.add_series(
        EMPTY_SERIES_NAME, [float(point["value"]) for point in points]
    )
...
def _add_metrics_cards(
    ...
    for index, metric in enumerate(metrics):
        ...
        _apply_export_font(title_paragraph.font, size=METRIC_LABEL_FONT_SIZE_PT)
        title_paragraph.alignment = PP_ALIGN.CENTER

        value_paragraph = frame.add_paragraph()
        value_paragraph.text = metric["value"]
        _apply_export_font(
            value_paragraph.font,
            size=METRIC_VALUE_FONT_SIZE_PT,
            bold=True,
        )
