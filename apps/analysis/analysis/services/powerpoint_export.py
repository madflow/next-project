...
def _set_data_label_number_format(data_label: Any, number_format: str) -> None:
    d_lbl = data_label._get_or_add_dLbl()  # noqa: SLF001
    d_lbl.remove_all("c:numFmt")

    num_fmt = OxmlElement("c:numFmt")
    num_fmt.set("formatCode", number_format)
    num_fmt.set("sourceLinked", "0")
    d_lbl.insert_element_before(
        num_fmt,
        "c:spPr",
        "c:txPr",
        "c:dLblPos",
...
def _add_chart_data_table(chart: Any) -> None:
    plot_area = chart._chartSpace.plotArea  # noqa: SLF001
    plot_area.remove_all("c:dTable")

    data_table = OxmlElement("c:dTable")
    for tag_name in (
        "c:showHorzBorder",
        "c:showVertBorder",
        "c:showOutline",
        ...
        option = OxmlElement(tag_name)
        option.set("val", "1")
        data_table.append(option)

    plot_area.insert_element_before(data_table, "c:spPr", "c:extLst")
...
def _add_stacked_bar_chart(slide: Any, rows: Sequence[ExportStackedRow]) -> None:
    ...
    for series_index, series in enumerate(chart.series):
        ...
        for point in series.points:
            ...
            _set_data_label_number_format(point.data_label, PERCENT_NUMBER_FORMAT)

    _add_chart_data_table(chart)
