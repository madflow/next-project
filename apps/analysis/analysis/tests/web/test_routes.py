...
def test_excel_export_request_model() -> None:
    """Test Excel export request validation."""
...
@pytest.mark.anyio
@patch("analysis.web.api.datasets.routes._get_dataset_by_id")
@patch("analysis.web.api.datasets.routes.build_workbook")
async def test_excel_export_endpoint_logic(
    mock_build_workbook: Mock,
    mock_get_dataset: Mock,
) -> None:
    """Test the Excel export route returns a binary response."""
    mock_dataset = Mock()
    mock_get_dataset.return_value = mock_dataset
    ...
    response = await export_dataset_excel(
        dataset_id="dataset-id",
        export_request=request,
        db=AsyncMock(),
        api_key="test-key",
    ...
    mock_build_workbook.assert_called_once_with(request.model_dump())
    assert (
        response.media_type
    ...
    assert (
        response.headers["Content-Disposition"]
        == 'attachment; filename="chart-export-2026-03-17.xlsx"'
    )
