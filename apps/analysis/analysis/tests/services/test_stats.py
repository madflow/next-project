...
def test_describe_var_numeric(stats_service, series, var_name, include, expected) -> None:
...
def test_describe_var_categorical(stats_service, series, var_name, include, expected) -> None:
...
def test_describe_var_edge_cases(stats_service, series, var_name, include, expected) -> None:
...
def test_frequency_table_with_value_labels_includes_all_valid_labels(stats_service) -> None:
...
def test_describe_var_with_split_variable_mixed_numeric_string_categories(stats_service) -> None:
...
def test_split_variable_not_found_error(stats_service) -> None:
    ...
    with pytest.raises(ValueError, match=r"Split variable 'nonexistent' not found in the DataFrame\."):
...
def test_describe_var_with_split_variable_main_var_missing_values(stats_service) -> None:
