...
numeric_test_data = [
    ...
    (pd.Series([1, 2, 2, 3, 4, 4, 4]), 'numeric_var', ['count', 'mean', 'std', 'min', 'max', 'median', 'range', 'mode', 'frequencies'], {
        'count': 7,
        ...
        'mode': [4],
        'frequency_table': [
            {'value': 4, 'counts': 3, 'percentages': 42.857142857142854},
            {'value': 2, 'counts': 2, 'percentages': 28.57142857142857},
            {'value': 1, 'counts': 1, 'percentages': 14.285714285714285},
            {'value': 3, 'counts': 1, 'percentages': 14.285714285714285}
        ]
    }),
    # Case with floating point numbers
    (pd.Series([1.5, 2.5, 2.5, 3.5]), 'float_var', ['mean', 'std'], {
        'mean': 2.5,
        'std': 0.816496580927726
    }),
    # Case with negative numbers
    (pd.Series([-1, -2, -3, -2]), 'neg_var', ['min', 'max', 'range'], {
        'min': -3.0,
        'max': -1.0,
        'range': 2.0
    }),
    # Case with a single value
    (pd.Series([5]), 'single_value_var', None, {
        'count': 1,
        'mean': 5.0,
        'std': None,
        'min': 5.0,
        ...
        'median': 5.0,
        'range': 0.0,
        'frequency_table': [{'value': 5, 'counts': 1, 'percentages': 100.0}]
    }),
]
    ...
    for key, value in expected.items():
        if isinstance(value, float):
            ...
            for res_item, exp_item in zip(sorted_result, sorted_expected):
                assert res_item['value'] == exp_item['value']
...
categorical_test_data = [
    ...
    (pd.Series(['apple', 'banana', 'apple', 'orange', 'banana', 'apple']), 'fruit_var', ['count', 'mode', 'frequencies'], {
        'count': 6,
        'mode': ['apple'],
        'frequency_table': [
    ...
    (pd.Series([1, 'apple', 2, 'apple']), 'mixed_var', None, {
        'count': 4,
        'mode': ['apple'],
    }),
    # Case with a single category
    (pd.Series(['A', 'A', 'A']), 'single_cat_var', ['count', 'mode', 'frequencies'], {
        'count': 3,
        'mode': ['A'],
...
edge_case_data = [
    ...
    (pd.Series([None, None, None], dtype=float), 'nan_var', None, {
        'count': 0,
        'mean': None,
        'std': None,
        'min': None,
    ...
    (pd.Series([1, 2, None, 4]), 'some_nan_var', ['count', 'mean', 'min', 'max'], {
        'count': 3,
    ...
    # Empty series
    (pd.Series([], dtype=float), 'empty_var', None, {
        'count': 0,
        'mean': None,
        'std': None,
        'min': None,
