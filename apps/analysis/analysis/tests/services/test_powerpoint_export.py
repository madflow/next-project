...
def test_build_presentation_for_distribution_uses_blank_series_name() -> None:
    """Single-series chart exports should keep the series title blank."""
    payload = {
        "file_name": "bar-export-2026-03-17.pptx",
        "title": "Age group",
        "meta_line": "Dataset: Survey 2026 | Exported: Mar 17, 2026",
...
def test_build_presentation_uses_arial_for_slide_and_chart_text() -> None:
    """PowerPoint exports should apply Arial to slide text and chart labels."""
    payload = {
    ...
    chart = cast(Any, slide.shapes[2]).chart
    assert (
        cast(Any, slide.shapes[0]).text_frame.paragraphs[0].font.name
        == EXPORT_FONT_NAME
    )
    assert (
        cast(Any, slide.shapes[1]).text_frame.paragraphs[0].font.name
        == EXPORT_FONT_NAME
    )
...
def test_build_presentation_for_stacked_chart_uses_contrast_label_color() -> None:
    ...
    assert chart.plots[0].data_labels.number_format_is_linked is False
