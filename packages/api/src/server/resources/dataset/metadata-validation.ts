...
const allowedMetadataFileExtensions = Object.keys(metadataMimeTypes) as (keyof typeof metadataMimeTypes)[];

type MetadataFileExtension = (typeof allowedMetadataFileExtensions)[number];
...
export type MetadataFileValidationResult = MetadataFileValidationSuccess | MetadataFileValidationFailure;

const allowedDetectedExtensions = new Set<MetadataFileExtension>(["jpeg", "jpg", "pdf", "png", "webp"]);
...
export async function validateMetadataFile(file: File): Promise<MetadataFileValidationResult> {
  ...
  if (!originalExtension || !allowedMetadataFileExtensions.includes(originalExtension as MetadataFileExtension)) {
    return {
      success: false,
  ...
  } else if (fileType?.ext && allowedDetectedExtensions.has(fileType.ext as MetadataFileExtension)) {
