...
test.describe("Adhoc Analysis - Basic Functionality", () => {
  test("should navigate to adhoc analysis, select SPSS Beispielumfrage dataset, and attempt to expand Demografische Daten and select Alter variable", async ({ page }) => {
    // Login as admin
    await page.goto("/");
    ...
    if (await demografischeGroup.count() > 0) {
      ...
      if (await alterVariable.count() > 0) {
        ...
        const anyChart = page.locator('[data-testid*="chart"], [data-testid*="analysis"], [data-testid*="visualization"]').first();
      ...
      if (await anyVariableGroup.count() > 0) {
        console.log(`ℹ "Demografische Daten" not found, but other variable groups available for dataset: SPSS Beispielumfrage`);
        ...
        console.log(`ℹ No variable groups available for dataset: SPSS Beispielumfrage (dataset may still be processing)`);
      }
    }
