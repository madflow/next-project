  ...
  test("should select SPSS Beispielumfrage dataset and test split functionality", async ({ page }) => {
    ...
    if (await anyVariableGroup.count() > 0) {
      ...
      for (let i = 0; i < groupCount; i++) {
        ...
        for (let j = 0; j < Math.min(variableCount, 3); j++) { // Log first 3 variables
        ...
        const commonSplitVariables = [
          "Geschlecht", "Gender", "Alter", "Age", "Familienstand", 
          "Marital", "Education", "Bildung", "Beruf", "Occupation"
        ];
      ...
      if (targetVariableFound) {
        ...
        if (await splitButton.count() > 0) {
          ...
          if (await splitByText.count() > 0) {
            ...
            console.log("⚠ Split functionality not found - this might indicate the feature is not yet implemented or uses different selectors");
          }
        }
