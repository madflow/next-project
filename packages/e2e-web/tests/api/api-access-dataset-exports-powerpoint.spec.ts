...
const TEST_ENDPOINT = `/api/datasets/${testIds.datasets.primary}/exports/powerpoint`;
...
test.describe("API dataset PowerPoint export access @api", () => {
  ...
  test("allows access for regular user with dataset access", async ({ page }) => {
    ...
    expect(response.headers()["content-disposition"]).toContain('attachment; filename="chart-export-2026-03-17.pptx"');
